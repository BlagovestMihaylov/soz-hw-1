...
{
    /*GA PARAMETERS*/
    private final int MAX_LENGTH;                 // chess board width. or n in n queens
    private final int START_SIZE;                    // Population size at start.
    private int MAX_EPOCHS;                 // Arbitrary number of test cycles.
    private final double MATING_PROBABILITY;      // Probability of two chromosomes mating. Range: 0.0 < MATING_PROBABILITY < 1.0
    private double MUTATION_RATE;           // Mutation Rate. Range: 0.0 < MUTATION_RATE < 1.0
    private final int MIN_SELECT;                 // Minimum parents allowed for selection.
    private final int MAX_SELECT;                 // Maximum parents allowed for selection. Range: MIN_SELECT < MAX_SELECT < START_SIZE
    private final int OFFSPRING_PER_GENERATION;   // New offspring created per generation. Range: 0 < OFFSPRING_PER_GENERATION < MAX_SELECT.
    private final int MINIMUM_SHUFFLES;           // For randomizing starting chromosomes
    ...
    public void getFitness()
    {
        ...
        for (Chromosome chromosome : population)
        {
            ...
            double fitness = (worstScore - thisChromo.getConflicts()) * 100.0 / bestScore;
